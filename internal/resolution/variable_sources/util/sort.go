...
func ByChannelAndVersion(entity1 *input.Entity, entity2 *input.Entity) bool {
	e1 := olm.NewBundleEntity(entity1)
	e2 := olm.NewBundleEntity(entity2)
...
func packageOrder(e1, e2 *olm.BundleEntity) int {
...
func channelOrder(e1, e2 *olm.BundleEntity) int {
	name1, err1 := e1.Channel()
	name2, err2 := e2.Channel()
	errComp := compareErrors(err1, err2)
	if errComp != 0 {
		return errComp
	}
...
func versionOrder(e1, e2 *olm.BundleEntity) int {
