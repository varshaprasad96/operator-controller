...
type RequiredPackageVariable struct {
	*input.SimpleVariable
	bundleEntities []*olm.BundleEntity
}

func (r *RequiredPackageVariable) BundleEntities() []*olm.BundleEntity {
	return r.bundleEntities
}

func NewRequiredPackageVariable(packageName string, bundleEntities []*olm.BundleEntity) *RequiredPackageVariable {
...
func (r *RequiredPackageVariableSource) GetVariables(ctx context.Context, entitySource input.EntitySource) ([]deppy.Variable, error) {
	resultSet, err := entitySource.Filter(ctx, util.WithPackageName(r.packageName))
	if err != nil {
		return nil, err
	}
	if len(resultSet) == 0 {
	...
	resultSet = resultSet.Sort(util.ByChannelAndVersion)
	var bundleEntities []*olm.BundleEntity
	for i := 0; i < len(resultSet); i++ {
		bundleEntities = append(bundleEntities, olm.NewBundleEntity(&resultSet[i]))
	}
