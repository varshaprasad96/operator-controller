...
// MakeRequiredPackageVariablesForExtensions returns a variable which represent
// explicit requirement for a package from an user.
// This is when a user explicitly asks "install this" via Extension API.
func MakeRequiredPackageVariablesForExtensions(allBundles []*catalogmetadata.Bundle, extensions []ocv1alpha1.Extension) ([]*olmvariables.RequiredPackageVariable, error) {
	result := make([]*olmvariables.RequiredPackageVariable, 0, len(extensions))
