...
func (r *ExtensionReconciler) variables(ctx context.Context) ([]deppy.Variable, error) {
	...
	// Filter apps belogining to current namespace only to create install variables?
	// This would ignore cluster scoped APIs installed from other Apps.
	appList := kappctrlv1alpha1.AppList{}
	if err := r.Client.List(ctx, &appList); err != nil {
		return nil, err
	}

	return GenerateVariablesForApp(allBundles, extensionList.Items, appList.Items)
}
